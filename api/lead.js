// Reçoit le formulaire de la landing acheteur et crée le lead dans GoHighLevel.
// Même mécanique que la page contact QR (projet ashkan-contact-info), adaptée aux champs de qualification.
...
const SOURCE = 'landing-acheteur';
const ETIQUETTES_BASE = ['acheteur', 'landing-acheteur'];
...
const BUDGETS_MAX = ['350 000', '400 000', '450 000', '500 000', '600 000', '700 000', '800 000', '1 000 000', '1 500 000', '2 000 000+'];
const DELAIS = { Immediat: 'délai-immédiat', '1-3 mois': 'délai-1-3-mois', '3-6 mois': 'délai-3-6-mois', 'Je magasine': 'délai-magasine' };
...
const ORIGINES = [
  /^https:\/\/ashkan-landing(-f8dw|-ojbl)?\.vercel\.app$/,
  /^https:\/\/ashkan-landing(-f8dw|-ojbl)?(-(git-[a-z0-9-]+|[a-z0-9]+))?-omarrerosarduy-1283s-projects\.vercel\.app$/,
...
export default async function handler(req, res) {
  ...
  const journal = { evt: 'lead', page: lead.page, etapes: {} };
  try {
    ...
    const alerte =
      `Nouveau lead (landing acheteur) : ${nomAffichable(lead.prenom)} ${nomAffichable(lead.nom)}, ${telephoneLisible(lead.telephone)}. ` +
    ...
    const [n, t, ack, alr, suivi] = await Promise.allSettled([
      ...
      accuserReception(contact.id, lead, 'vous contactera sous peu pour discuter de votre recherche.'),
...
function valider(c) {
  ...
  return {
    ...
    page: c.page === 'acheteur' ? 'acheteur' : 'accueil',
    utm: lireUtm(c.utm),
  };
}
...
async function trouverOuCreerContact(lead) {
  ...
  const fiche = {
    ...
    customFields: [{ id: CHAMPS.langues, field_value: 'Vouvoiement' }, ...champsCriteres(lead)],
  };
  try {
...
async function creerOpportunite(contactId, lead) {
  ...
  if (enCours.length) return 'dossier déjà en cours dans OZ - Acheteur, laissé à son étape';

  const valeur = Number((lead.budgetMax || '').replace(/\D/g, '')) || undefined;
  await ghl('POST', '/opportunities/', {
    locationId: LOCATION_ID,
    pipelineId: PIPELINE.id,
    pipelineStageId: PIPELINE.etape,
    contactId,
    name: `${lead.prenom} ${lead.nom} — Achat | Landing`,
    status: 'open',
    assignedTo: ASHKAN_USER_ID,
    ...(valeur ? { monetaryValue: valeur } : {}),
  });
...
function construireNote(lead, contact, oppInfo, ref, majCoordonnees) {
  const lignes = [
    `Demande reçue par la landing acheteur (page ${lead.page}), le ${horodatage()}. Réf. ${ref}`,
...
function construireTache(lead, ref) {
  return {
    title: `Rappeler ${lead.prenom} ${lead.nom} (landing acheteur)`,
    body: [`Demande reçue par la landing acheteur. Réf. ${ref}`, ...texteVisiteur(lead.message)].join('\n'),
...
async function accuserReception(contactId, lead, texteMilieu) {
  if (!lead.telephone.startsWith('+1')) return 'non envoyé (numéro hors Amérique du Nord)';
  const prenom = nomAffichable(lead.prenom);
  await envoyerSms(
    contactId,
    `Bonjour${prenom ? ` ${prenom}` : ''}, merci pour votre demande. Ashkan Javid, courtier immobilier chez Royal LePage Urbain, ${texteMilieu} Répondez STOP pour ne plus recevoir de textos.`,
  );
  return 'envoyé';
}
